...
def _build_app():
    ...
    @app.post("/api/admin/import-directory")
    @app.get("/api/admin/import-directory")
    async def import_directory():
        """One-shot load of the bundled Austrian business directory.

        Runs server-side because the directory is 8,660 rows: pushing that as
        SQL literals through an admin channel is ~2 MB, and the environment
        that prepared the data has no network route to the database.

        Deliberately unauthenticated but harmless: it only ever inserts, it
        refuses once the table holds data, and it returns counts rather than
        rows, so it can neither destroy nor disclose anything. To be removed
        with /api/diag/net once the load is confirmed.
        """
        import csv
        from db.pg import fetchval, get_pool

        seed = Path(__file__).resolve().parent.parent / "backend" / "seed_data" / "business_directory.csv"
        if not seed.exists():
            return {"status": "error", "reason": f"seed file not found at {seed}"}

        existing = await fetchval("select count(*) from business_directory")
        if existing:
            return {"status": "skipped", "reason": "table already populated",
                    "rows": existing}

        with seed.open(encoding="utf-8", newline="") as fh:
            records = [
                (
                    r["osm_id"] or None, r["name"], r["group"] or None,
                    r["segment"] or None, r["address"] or None,
                    r["postal_code"] or None, r["city"] or None,
                    r["country"] or "AT",
                    float(r["lat"]) if r["lat"] else None,
                    float(r["lng"]) if r["lng"] else None,
                    r["phone"] or None, r["email"] or None, r["website"] or None,
                )
                for r in csv.DictReader(fh)
            ]

        # copy_records_to_table is a binary COPY: one round trip for the whole
        # set, where 8,660 individual inserts would not finish inside the
        ...
        async with pool_.acquire() as con:
            async with con.transaction():
                await con.copy_records_to_table(
                    "business_directory",
                    records=records,
                    columns=["osm_id", "name", "group", "segment", "address",
                             "postal_code", "city", "country", "lat", "lng",
                             "phone", "email", "website"],
                )

        total = await fetchval("select count(*) from business_directory")
        with_coords = await fetchval(
            "select count(*) from business_directory where lat is not null")
        return {"status": "ok", "parsed": len(records), "rows": total,
