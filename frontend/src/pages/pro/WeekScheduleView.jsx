  ...
  return (
    <div data-testid="week-view">
      ...
      <div className="flex rounded-[12px] border border-sm-border bg-paper p-2 pb-1.5"
        ...
        <div className="w-[24px] flex-none relative" style={{ height: HEAD_H + GRID_H }}>
          {Array.from({ length: SPAN / 2 + 1 }, (_, i) => DAY_FROM + i * 2).map((h) => (
            <span key={h} className="absolute right-[3px] font-bold text-[0.53125rem] text-ink-faint"
          ...
          {days.map((d) => {
            ...
            return (
              ...
              >
                ...
                <span className={`absolute left-0 right-0 top-0 text-center font-bold text-[0.5625rem]
                  h-[48px] ${isToday ? 'text-teal' : 'text-ink-muted'}`}>
                  {d.toLocaleDateString(lang, { weekday: 'short' })}
                  <b className={`block font-extrabold text-[0.6875rem] ${isToday ? 'text-teal' : 'text-ink'}`}>
                    {d.getDate()}
                  </b>
                  {wxDay && <b className="block text-[0.5rem] font-bold text-sky-deep">
      ...
      <div className="flex items-baseline gap-2 mt-3.5 mb-1.5">
        <p className="font-headings font-bold text-[0.78125rem] text-ink" data-testid="week-sel-title">
          ...
          {i > 0 && selectedRuns
            ...
            .map((r) => (
              <div key={+r.start} className="flex items-center gap-2 ml-[53px] mb-1.5">
                ...
                <span className="font-bold text-[0.59375rem] text-ink-faint" data-testid="week-gap">
