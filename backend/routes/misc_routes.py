...
from routes.push_routes import notify_new_review
from models import (UserProfileUpdate, ProProfileUpdate, AvailabilityUpdate,
                    TranslateRequest, ReviewCreate, ReviewResponseCreate, BookingCreate)
...
# SAVED PROS
# ──────────────────────────────────────────────
@router.get("/saved-pros")
async def list_saved_pros(user: dict = Depends(get_current_user)):
    saved = await db.saved_pros.find({"homeowner_id": user["_id"]}).to_list(100)
    result = []
    for s in saved:
        pro = await db.pro_profiles.find_one({"_id": ObjectId(s["pro_id"])})
        if pro:
            p = serialize_doc(pro)
            ...
            if u:
                ...
                p["city"] = u.get("city", "")
            result.append(p)
    return {"saved_pros": result}


@router.post("/saved-pros/{pro_id}")
async def save_pro(pro_id: str, user: dict = Depends(get_current_user)):
    try:
        await db.saved_pros.insert_one({
        ...
        pass  # Already saved
    return {"message": "Pro saved"}


@router.delete("/saved-pros/{pro_id}")
async def unsave_pro(pro_id: str, user: dict = Depends(get_current_user)):
    await db.saved_pros.delete_one({"homeowner_id": user["_id"], "pro_id": pro_id})
    return {"message": "Pro removed from saved"}
...
async def create_review(data: ReviewCreate, user: dict = Depends(get_current_user)):
    if user["role"] != "homeowner":
        raise HTTPException(status_code=403, detail="Only homeowners can review")
    ...
    existing = await db.reviews.find_one({"homeowner_id": user["_id"], "job_id": data.job_id})
    if existing:
        raise HTTPException(status_code=400, detail="Already reviewed this job")

    pro_id = job.get("accepted_pro_id")
    if not pro_id:
        raise HTTPException(status_code=400, detail="No pro to review")
    ...
    review_doc = {
        ...
        "rating": data.rating,
        "text": data.text or "",
        "is_flagged": data.rating <= 2,
        "is_hidden": False,
        "created_at": datetime.now(timezone.utc)
    }
    inserted = await db.reviews.insert_one(review_doc)

    # Update pro rating
    all_reviews = await db.reviews.find({"pro_id": pro_id, "is_hidden": {"$ne": True}}).to_list(500)
    if all_reviews:
        avg = sum(r["rating"] for r in all_reviews) / len(all_reviews)
        await db.pro_profiles.update_one(
            {"_id": ObjectId(pro_id)},
            {"$set": {"rating_avg": round(avg, 1), "reviews_count": len(all_reviews)}}
        )

    # Web Push + in-app notification (respects pro's notif_new_review preference)
    try:
        pro_profile = await db.pro_profiles.find_one({"_id": ObjectId(pro_id)})
        if pro_profile and pro_profile.get("user_id"):
            homeowner_name = (user.get("name") or "").strip()
            # Privacy: respect reviewer's last-name flag
            show_last = user.get("privacy_show_lastname", True)
            if not show_last:
                parts = homeowner_name.split()
                homeowner_name = f"{parts[0]} {parts[-1][0]}." if len(parts) > 1 else homeowner_name
    ...
    # Notify admins about new reviews (all stars) so they can moderate if needed
    try:
        now = datetime.now(timezone.utc)
        admins = await db.users.find({"role": "admin"}, {"_id": 1}).to_list(10)
        if admins:
            stars_label = f"{data.rating}/5"
            await db.notifications.insert_many([
                {
                    "user_id": str(a["_id"]),
                    "type": "new_review",
                    "title": f"New {stars_label} Review {'(Low rating)' if data.rating <= 2 else ''}".strip(),
                    "body": f"{user.get('name','User')} — {(data.text or '')[:60]}",
...
# ──────────────────────────────────────────────
# REVIEW RESPONSES (Pro replies to their own reviews)
# ──────────────────────────────────────────────
@router.get("/reviews/mine")
async def get_my_reviews(user: dict = Depends(get_current_user)):
    """All reviews for the authenticated pro, sorted desc, with job context +
    response. Mirrors the public profile shape so the Pro settings page can
    show them and let the pro reply."""
    if user["role"] != "tradesperson":
    ...
    for rev in rows:
        ...
        if job_id:
            try:
                jd = await db.jobs.find_one({"_id": ObjectId(job_id)})
                if jd:
                    r["job_id"] = str(jd["_id"])
                    r["job_title"] = jd.get("title", "")
                    r["job_category"] = jd.get("category", "")
                    r["job_city"] = jd.get("city", "")
            except Exception:
                pass
        out.append(r)
    return {"reviews": out}
...
async def upsert_review_response(
    review_id: str, data: ReviewResponseCreate, user: dict = Depends(get_current_user)
):
    """Create or update the pro's response to a review. Only the reviewed pro
    can reply, one response per review."""
    if user["role"] != "tradesperson":
        raise HTTPException(status_code=403, detail="Pros only")
    try:
        rev = await db.reviews.find_one({"_id": ObjectId(review_id)})
    except Exception:
    ...
    pro_profile = await db.pro_profiles.find_one({"user_id": user["_id"]})
    if not pro_profile or str(pro_profile["_id"]) != rev.get("pro_id"):
        raise HTTPException(status_code=403, detail="Not authorized")
    ...
    if not rev.get("response_created_at"):
        update["response_created_at"] = now
    await db.reviews.update_one({"_id": ObjectId(review_id)}, {"$set": update})
    return {"message": "Response saved",
            "response": update["response"],
            "response_created_at": (rev.get("response_created_at") or now).isoformat(),
            "response_updated_at": now.isoformat()}


@router.delete("/reviews/{review_id}/response")
async def delete_review_response(review_id: str, user: dict = Depends(get_current_user)):
    if user["role"] != "tradesperson":
        raise HTTPException(status_code=403, detail="Pros only")
    try:
        rev = await db.reviews.find_one({"_id": ObjectId(review_id)})
    ...
    await db.reviews.update_one(
        ...
        {"$unset": {"response": "", "response_created_at": "", "response_updated_at": ""}},
    )
